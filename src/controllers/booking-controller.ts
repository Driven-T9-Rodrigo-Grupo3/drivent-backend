...
export async function listBookingByRoom(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  const { roomId } = req.params;
  try {
    const bookings = await bookingService.getBookingRoomById(Number(roomId));
